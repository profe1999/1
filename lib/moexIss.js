...
/**
 * Ищет ИМЕННО блок с позициями физ/юр лиц — по наличию колонок "clgroup" и
 * "ticker" (без учёta регистра — сравнение по имени показало, что блок
 * "futoi" не совпал по строгому регистру в прошлый раз, а MOEX ISS нередко
 * отдаёт колонки то в верхнем, то в нижнем регистре в разных продуктах).
 * Дополнительно всегда собираем структуру ВСЕХ блоков в диагностику — даже
 * если matching не сработает снова, в следующий раз будут видны настоящие
 * имена колонок вместо очередной догадки.
 */
function findDataBlock(json) {
  ...
  const blockDetails = blockNames.map((name) => {
    ...
    return {
      name,
      columns: b && Array.isArray(b.columns) ? b.columns : null,
      rowCount: b && Array.isArray(b.data) ? b.data.length : null
    };
  });
